...
"""
from pydantic import BaseModel, Field, validator, root_validator, constr, confloat, conint
...
class ArtistBase(BaseModel):
    ...
    # Validators
    _validate_country = validator('country', allow_reuse=True)(validate_iso_country)
    _validate_popularity = validator('popularity_score', allow_reuse=True)(validate_popularity)
    ...
    def set_normalized_name(cls, v, values):
        ...
        if v is None and 'artist_name' in values:
            return values['artist_name'].lower().strip()
        return v
...
class TrackBase(BaseModel):
    ...
    # Validators
    _validate_track_id = validator('track_id', allow_reuse=True)(validate_track_id)
    _validate_bpm = validator('bpm', allow_reuse=True)(validate_bpm)
    _validate_popularity = validator('popularity_score', allow_reuse=True)(validate_popularity)

    @validator('normalized_title', always=True)
    def set_normalized_title(cls, v, values):
        """Auto-generate normalized_title from track_name if not provided"""
        if v is None and 'track_name' in values:
            return values['track_name'].lower().strip()
        return v
    ...
    @root_validator
    def validate_remix_consistency(cls, values):
        """Ensure remix fields are consistent"""
        is_remix = values.get('is_remix')
        remix_type = values.get('remix_type')

        if is_remix and remix_type is None:
            raise ValueError('remix_type must be specified when is_remix=True')

        if not is_remix and remix_type is not None:
...
class SetlistBase(BaseModel):
    ...
    def set_normalized_name(cls, v, values):
        ...
        if v is None and 'setlist_name' in values:
            return values['setlist_name'].lower().strip()
        return v

    @validator('dj_artist_name')
    ...
    def validate_time_consistency(cls, values):
        ...
        start_time = values.get('set_start_time')
        end_time = values.get('set_end_time')
...
class TrackAdjacency(BaseModel):
    ...
    # Validators
    _validate_track_1_id = validator('track_1_id', allow_reuse=True)(validate_track_id)
    _validate_track_2_id = validator('track_2_id', allow_reuse=True)(validate_track_id)

    @root_validator
    def validate_different_tracks(cls, values):
        """Ensure track_1 and track_2 are different"""
        track1_name = values.get('track_1_name')
        track2_name = values.get('track_2_name')
...
class TrackSource(BaseModel):
    ...
    track_id: constr(regex=r'^[a-f0-9]{16}$')
    source: DataSource
