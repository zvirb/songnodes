#!/usr/bin/env python3
"""
Direct database population script with mock data for Docker environment.
Uses connection settings from environment.
"""
...
DATABASE_URL = os.environ.get('DATABASE_URL', 'postgresql://musicdb_user:7D82_xqNs55tGyk@db-connection-pool:6432/musicdb')
parsed_url = urlparse(DATABASE_URL)

# Database connection from parsed URL
DB_CONFIG = {
...
def create_mock_data(conn):
    ...
    for artist, artist_id in artist_ids.items():
        ...
        for i in range(num_tracks):
            ...
            cur.execute("""
                INSERT INTO songs (title, artist_id, genre, bpm, key, source, created_at, updated_at)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s) RETURNING id
            """, (track_name, artist_id, genre, bpm, key, '1001tracklists', datetime.now(), datetime.now()))
