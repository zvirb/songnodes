...
CREATE INDEX IF NOT EXISTS idx_pipeline_metrics_run_stage ON pipeline_execution_metrics(run_id, stage);
CREATE INDEX IF NOT EXISTS idx_pipeline_metrics_timestamp ON pipeline_execution_metrics(measurement_timestamp DESC);

CREATE INDEX IF NOT EXISTS idx_data_lineage_run ON data_lineage(run_id);
CREATE INDEX IF NOT EXISTS idx_data_lineage_source_type ON data_lineage(source_type, extraction_timestamp);

CREATE INDEX IF NOT EXISTS idx_graph_validation_run ON graph_validation_results(run_id, validation_passed);
CREATE INDEX IF NOT EXISTS idx_graph_validation_playlist ON graph_validation_results(playlist_id);

CREATE INDEX IF NOT EXISTS idx_quality_metrics_run_pillar ON data_quality_metrics(run_id, pillar);
CREATE INDEX IF NOT EXISTS idx_quality_metrics_timestamp ON data_quality_metrics(measured_at DESC);

CREATE INDEX IF NOT EXISTS idx_source_log_run_domain ON source_extraction_log(run_id, website_domain);
CREATE INDEX IF NOT EXISTS idx_source_log_url_timestamp ON source_extraction_log(source_url, extraction_timestamp);
CREATE INDEX IF NOT EXISTS idx_source_log_success ON source_extraction_log(success, extraction_timestamp);

CREATE INDEX IF NOT EXISTS idx_impact_analysis_run_entity ON graph_impact_analysis(run_id, entity_type);
CREATE INDEX IF NOT EXISTS idx_impact_analysis_type_timestamp ON graph_impact_analysis(analysis_type, analysis_timestamp);

CREATE INDEX IF NOT EXISTS idx_anomaly_run_severity ON anomaly_detection(run_id, severity);
CREATE INDEX IF NOT EXISTS idx_anomaly_type_timestamp ON anomaly_detection(anomaly_type, detection_timestamp);
CREATE INDEX IF NOT EXISTS idx_anomaly_acknowledged ON anomaly_detection(acknowledged, severity);
...
DROP TRIGGER IF EXISTS update_scraping_run_end_time_trigger ON scraping_runs;
