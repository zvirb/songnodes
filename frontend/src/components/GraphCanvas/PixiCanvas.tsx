...
export const PixiCanvas: React.FC<PixiCanvasProps> = ({ width, height, nodes, edges, onNodeRightClick }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = x.onChange(val => { container.x = val; });
    const unsubscribeY = y.onChange(val => { container.y = val; });
    const unsubscribeScale = scale.onChange(val => { container.scale.set(val); });
